...
public class PhotoTools {
    ...
    public static List<PhotoFolderInfo> getVideos(Context context, List<PhotoInfo> selectPhotoMap) {
        List<PhotoFolderInfo> allPhotoFolderList = new ArrayList<>();
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                final int imageId = cursor.getInt(imageIdColumn);
                final String path = cursor.getString(dataColumn);
                final String videoThumbPath = cursor.getString(thumbImageColumn);
                //final String thumb = cursor.getString(thumbImageColumn);
                File file = new File(path);
                if ((filterList == null || !filterList.contains(path)) && file.exists() && file.length() > 0) {
                    final PhotoInfo photoInfo = new PhotoInfo();
